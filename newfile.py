...
def find_similar_sentences(sentences1, sentences2, threshold=0.5):
    ...
    vectorizer = TfidfVectorizer()
    try:
        # Vectorize sentences separately
...
if uploaded_files and len(uploaded_files) >= 2:
    ...
    if st.session_state.get('nltk_downloaded', False):
        ...
        with col1:
            st.subheader(
                "Preventing Financial Crime that Harms Sustainability")
            st.markdown(f"""
...
# --- Interesting Facts about ING Bank ---
        st.header("Interesting Facts about ING Bank")
        st.markdown(
            "**Sustainability Commitment:** ING has pledged to become climate neutral by 2050")
                        ...
                        with st.spinner("Running AI analysis..."):
                            try:
                                ...
                                if analysis_task == "Analyze Differences":
                                    ...
                                    system_message = "You are an AI assistant specialized in generating software test cases based on compliance documents. Based on the provided impact analysis report, generate Cucumber (Gherkin) test cases focusing on the identified differences and potential gaps. Ensure the test cases are relevant to SWIFT compliance changes. Provide only the Gherkin syntax in the output."
                                    prompt_text = f"Based on the following impact analysis report, generate Cucumber (Gherkin) test cases focusing on the identified differences and potential gaps:\n\n{impact_report_content}\n\nUser Request: {user_prompt}\n\nFormat the output strictly as Gherkin scenarios (Feature, Scenario, Given, When, Then, And, But)."
                                else:  # Custom Prompt
