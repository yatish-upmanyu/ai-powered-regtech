...
def preprocess_text(text):
    ...
    try:
        ...
        logging.error("NLTK stopwords not loaded. Cannot preprocess text.")
...
def get_sentences(text):
    ...
    try:
        ...
        logging.error(
            "NLTK punkt tokenizer not loaded. Cannot split into sentences.")
        return text.split('\n')
...
def find_similar_sentences(sentences1, sentences2, threshold=0.5):
    ...
    try:
        ...
        logging.error(f"Error finding similar sentences: {e}")
        return []
    return similar_pairs
...
def find_differences(sentences1, sentences2, threshold=0.2):
    ...
    try:
        ...
        logging.error(f"Error finding differences: {e}")
        return sentences1, sentences2
...
def visualize_clusters(clusters, sentences):
    if not clusters or not sentences or len(sentences) < 2:
        ...
        logging.warning("Not enough data to visualize clusters.")
        return
    ...
    try:
        ...
        try:
            ...
            logging.warning(f"PCA failed: {e}. Cannot visualize clusters.")
            return
...
def generate_impact_report_content(doc_names, overall_similarity, similar_pairs, diff1, diff2, keywords1, keywords2, gap_report, similarity_threshold, difference_threshold):
    ...
    report_content += f"*{st.session_state.get('app_version', 'v0.6')} - For informational purposes only. Consult official SWIFT documentation and legal counsel for definitive compliance guidance.*"
...
if 'app_version' not in st.session_state:
    st.session_state.app_version = "v0.6"

# --- Streamlit UI ---
st.title("CompliWise AI")
            ...
            for uploaded_file in uploaded_files:
                if uploaded_file is not None:
                    ...
                    if raw_text:
                        ...
                        logging.warning(
                            f"Could not extract text from {doc_name}.")
                        docs[doc_name] = {"raw": "",
                                          "processed": "", "sentences": []}
