...
def find_similar_sentences(sentences1, sentences2, threshold=0.5):
    ...
    try:
        ...
        len1 = len(sentences1)
        similarity_matrix = cosine_similarity(tfidf_matrix[:len1], tfidf_matrix[len1:])
        for i in range(len(sentences1)):
            for j in range(len(sentences2)):
                if similarity_matrix[i, j] > threshold:
                    similar_pairs.append((sentences1[i], sentences2[j], similarity_matrix[i, j]))
        similar_pairs.sort(key=lambda x: x[2], reverse=True)
    except ValueError:
         st.warning("Could not compute similarity, possibly due to empty documents after preprocessing.")
         return []
    return similar_pairs
...
def find_differences(sentences1, sentences2, threshold=0.2):
    ...
    try:
        ...
        st.warning("Could not compute differences, possibly due to empty documents after preprocessing.")
        return sentences1, sentences2
...
def visualize_clusters(clusters, sentences):
    ...
    try:
        ...
        st.warning("Could not visualize clusters, possibly due to issues with data dimensionality.")
...
st.markdown("Upload two or more SWIFT-related compliance documents (PDF or TXT) to analyze and compare them.")
...
with st.sidebar:
    ...
    similarity_threshold = st.slider("Similarity Threshold", 0.1, 1.0, 0.6, 0.05, help="Minimum cosine similarity score to consider sentences 'similar'.")
    difference_threshold = st.slider("Difference Threshold", 0.0, 0.5, 0.2, 0.05, help="Maximum cosine similarity score for a sentence to be considered 'different' from the other document.")
    summary_length = st.slider("Summary Sentences", 3, 10, 5, 1, help="Number of sentences for extractive summaries.")
    num_clusters = st.slider("Number of Clusters (for visualization)", 2, 10, 5, 1, help="Number of clusters to group sentences into.")
...
if uploaded_files and len(uploaded_files) >= 2:
        ...
        for uploaded_file in uploaded_files:
            if uploaded_file is not None:
                ...
                if raw_text:
                    ...
                    docs[doc_name] = {"raw": "", "processed": "", "sentences": []}
        ...
        if doc1 and doc2:
            ...
            with col1:
                ...
                with st.expander("View Full Text", expanded=False):
                    st.text_area("Doc1 Raw", doc1['raw'], height=200, key="raw1")
            with col2:
                st.subheader(f"Document 2: {doc2_name}")
                with st.expander("View Full Text", expanded=False):
                    st.text_area("Doc2 Raw", doc2['raw'], height=200, key="raw2")

            st.divider()

            with st.spinner("Analyzing..."):
                overall_similarity = calculate_similarity(doc1['processed'], doc2['processed'])
                similar_pairs = find_similar_sentences(doc1['sentences'], doc2['sentences'], similarity_threshold)
                diff1, diff2 = find_differences(doc1['sentences'], doc2['sentences'], difference_threshold)
                keywords1 = {cat: identify_keywords(doc1['raw'], kws) for cat, kws in KEYWORDS.items()}
                keywords2 = {cat: identify_keywords(doc2['raw'], kws) for cat, kws in KEYWORDS.items()}

            st.metric("Overall Document Similarity", f"{overall_similarity:.2f}")
            st.markdown("---")

            st.subheader("Key Similarities")
            if similar_pairs:
                st.markdown(f"Found **{len(similar_pairs)}** similar sentence pairs (similarity > {similarity_threshold}):")
                ...
                st.info(f"No significant sentence similarities found above the threshold ({similarity_threshold}).")

            st.subheader("Key Differences / Unique Content")
            col_d1, col_d2 = st.columns(2)
            with col_d1:
                st.markdown(f"**Potentially Unique to '{doc1_name}'** (Low similarity < {difference_threshold} to '{doc2_name}')")
                if diff1:
                    with st.expander(f"Show {len(diff1)} sentences", expanded=False):
                        for sent in diff1:
                            st.warning(sent)
                else:
                    st.info("No significant unique sentences identified based on the threshold.")
            with col_d2:
                st.markdown(f"**Potentially Unique to '{doc2_name}'** (Low similarity < {difference_threshold} to '{doc1_name}')")
                if diff2:
            ...
            for category, kws in KEYWORDS.items():
                with st.expander(f"Category: {category}", expanded=False):
                    ...
                    common_kws = set(kw for kw, sents in cat_key1.items() if sents) & set(kw for kw, sents in cat_key2.items() if sents)
                    unique_kws1 = set(kw for kw, sents in cat_key1.items() if sents) - set(kw for kw, sents in cat_key2.items() if sents)
                    unique_kws2 = set(kw for kw, sents in cat_key2.items() if sents) - set(kw for kw, sents in cat_key1.items() if sents)

                    if common_kws:
                        st.success(f"Common Keywords: {', '.join(common_kws)}")
                    if unique_kws1:
                        st.warning(f"Keywords mainly in {doc1_name}: {', '.join(unique_kws1)}")
                    if unique_kws2:
                        st.warning(f"Keywords mainly in {doc2_name}: {', '.join(unique_kws2)}")
                    if not common_kws and not unique_kws1 and not unique_kws2:
                        st.info("No keywords found in this category for either document.")
        else:
            st.warning("Could not process one or both of the selected documents.")

    # --- Summary & Gaps Analysis Tab ---
    with tabs[1]:
        st.header("Summary & Gaps Analysis")
        st.markdown("This section provides summaries and highlights potential gaps.")

        st.subheader("Document Summaries")
        for name, data in docs.items():
            if data['raw']:
                st.markdown(f"**Summary for '{name}'** ({summary_length} sentences):")
        ...
        st.markdown("Highlights keywords present in some documents but not others.")
        all_keywords_found = {}
        for category, kws in KEYWORDS.items():
            all_keywords_found[category] = {}
            for kw in kws:
                present_in = [name for name, data in docs.items() if data['raw'] and kw.lower() in data['raw'].lower()]
                if present_in:
        ...
        for category, keywords_data in all_keywords_found.items():
            for keyword, present_in_docs in keywords_data.items():
                if 0 < len(present_in_docs) < num_docs:
                    missing_in_docs = list(set(doc_names) - set(present_in_docs))
                    gap_report.append({
                        "Category": category,
    ...
    with tabs[2]:
        ...
        all_sentences_from_docs = [sent for name, data in docs.items() for sent in data['sentences']]

        if all_sentences_from_docs:
            with st.spinner("Clustering..."):
                clusters, summary = cluster_sentences(all_sentences_from_docs, num_clusters)
                st.info(summary)
    ...
    with tabs[3]:
        ...
        """, unsafe_allow_html=True)
        st.subheader("Strengthening Financial Stability for Sustainable Development")
        st.markdown("""
    ...
    with tabs[4]:
        ...
        st.markdown("This section demonstrates potential integration with Generative AI for deeper insights.")
        ...
        if len(docs) >= 2:
            ...
            if doc1_raw_text and doc2_raw_text:
                ...
                st.markdown("Click the button below to analyze the documents for contradictory statements using a Generative AI model.")
                if st.button("Analyze for Contradictions"):
                    with st.spinner("Analyzing for contradictions using Gen AI..."):
                        ...
                        st.info("Contradiction analysis using a Generative AI model would be displayed here. This requires integration with a Gen AI platform's API.")

                st.subheader("Generate Comparative Summary")
                st.markdown("Click the button to generate a summary highlighting the key similarities and differences between the documents using Gen AI.")
                if st.button("Generate Comparative Summary"):
                    with st.spinner("Generating comparative summary using Gen AI..."):
                        ...
                        st.info("A comparative summary generated by a Generative AI model would be shown here.")

                st.subheader("Ask a Comparative Question")
                question = st.text_input("Enter your question about the documents (e.g., 'What are the key differences in security protocols?'):")
                if st.button("Ask Question"):
                    with st.spinner("Answering your question using Gen AI..."):
                        ...
                        st.info("The answer to your question from a Generative AI model would appear here.")
            else:
                st.warning("Please ensure both documents have extracted text for intelligent analysis.")
        else:
            st.warning("Please upload at least two documents for intelligent analysis.")
